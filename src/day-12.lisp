(defpackage #:aoc-2023/day-12
  ...
  (:export #:hot-springs-1
           #:hot-springs-2)
  (:import-from #:aoc-2023/day-12-input
                #:*input*))
...
(defun calculate-arrangements-rec-inner (row row-pos conditions conditions-pos grp-count)
  (if (gethash (+ (ash row-pos 50) (ash conditions-pos 25) grp-count) *memo*)
      (gethash (+ (ash row-pos 50) (ash conditions-pos 25) grp-count) *memo*)
      (let ((result
              (if (= row-pos (length row))
                  (if (and (= conditions-pos (length conditions)) (= 0 grp-count))
                      1
                      0)
                  (loop :for chr :in (if (char= (char row row-pos) #\?) (list #\# #\.) (list (char row row-pos)))
                        :sum (if (char= chr #\#)
                                 (calculate-arrangements-rec-inner row (1+ row-pos) conditions conditions-pos (1+ grp-count))
                                 (if (/= 0 grp-count)
                                     (if (and (/= conditions-pos (length conditions))
                                              (= grp-count (aref conditions conditions-pos)))
                                         (calculate-arrangements-rec-inner row (1+ row-pos) conditions (1+ conditions-pos) 0)
                                         0)
                                     (calculate-arrangements-rec-inner row (1+ row-pos) conditions conditions-pos 0)))))))
        (setf (gethash (+ (ash row-pos 50) (ash conditions-pos 25) grp-count) *memo*) result)
        result)))

(defun calculate-arrangements-rec (row conditions)
  (setq *memo* (make-hash-table :test #'eq))
  (calculate-arrangements-rec-inner row 0 conditions 0 0))
...
(defun hot-springs-1 (&optional (stream (make-string-input-stream *input*)))
  "8180"
  (sum-arrangements (parse stream 1)))

(defun hot-springs-2 (&optional (stream (make-string-input-stream *input*)))
  "620189727003627"
