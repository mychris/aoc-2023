(defpackage #:aoc-2023/day-14
  ...
  (:import-from #:aoc-2023/day-14-input
                #:*input*))
...
(defun parabolic-reflector-dish-1 (&optional (stream (make-string-input-stream *input*)))
  "106990"
  (multiple-value-bind (matrix width height)
      (parse-string stream)
    (load-north (tilt-north matrix width height) width height)))

(defun parabolic-reflector-dish-2 (&optional (stream (make-string-input-stream *input*)))
  "100531"
  (multiple-value-bind (matrix width height)
      (parse-string stream)
    (let ((cache nil)
