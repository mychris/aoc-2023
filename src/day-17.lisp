(defpackage #:aoc-2023/day-17
  ...
  (:export #:clumsy-crucible-1
           #:clumsy-crucible-2)
  (:import-from #:aoc-2023/day-17-input
                #:*input*))
...
(defun clumsy-crucible-1 (&optional (stream (make-string-input-stream *input*)))
  "859"
  (multiple-value-bind (map width)
      (parse stream)
    (find-path map width 0 (1- (length map)) 1 3)))

(defun clumsy-crucible-2 (&optional (stream (make-string-input-stream *input*)))
  "1027"
  (multiple-value-bind (map width)
      (parse stream)
